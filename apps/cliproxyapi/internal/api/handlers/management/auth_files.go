...
func authNoteValue(auth *coreauth.Auth) string {
	...
	if note := authMetadataString(auth.Metadata, "note"); note != "" {
		return note
	}
	if auth.Attributes != nil {
...
func (h *Handler) authPathForRecord(record *coreauth.Auth) string {
	...
	if id := strings.TrimSpace(record.ID); id != "" {
		...
		if h != nil && h.cfg != nil && strings.TrimSpace(h.cfg.AuthDir) != "" {
			return filepath.Join(h.cfg.AuthDir, id)
		}
	}
	return ""
}

func (h *Handler) existingAuthNoteForRecord(record *coreauth.Auth) string {
	if record == nil {
		return ""
	}
	...
	if h != nil && h.authManager != nil {
		...
		for _, candidate := range candidates {
			...
			if auth, ok := h.authManager.GetByID(candidate); ok {
				if note := authNoteValue(auth); note != "" {
					return note
				}
			}
		}
		...
		if fileName != "" {
			for _, auth := range h.authManager.List() {
				...
				if !strings.EqualFold(strings.TrimSpace(auth.FileName), fileName) {
					continue
				}
				if note := authNoteValue(auth); note != "" {
					return note
	...
	return authMetadataString(metadata, "note")
}

func codexDefaultWorkspaceNote(record *coreauth.Auth) string {
	if record == nil || !strings.EqualFold(strings.TrimSpace(record.Provider), "codex") {
		return ""
	}

	idToken := authMetadataString(record.Metadata, "id_token")
	workspaceIDCandidates := make([]string, 0, 5)
	for _, key := range []string{"organization_id", "workspace_id", "account_id"} {
		if value := authMetadataString(record.Metadata, key); value != "" {
			workspaceIDCandidates = append(workspaceIDCandidates, value)
		}
	}

	if storage, ok := record.Storage.(*codex.CodexTokenStorage); ok && storage != nil {
		if idToken == "" {
			idToken = strings.TrimSpace(storage.IDToken)
		}
		if accountID := strings.TrimSpace(storage.AccountID); accountID != "" {
			workspaceIDCandidates = append(workspaceIDCandidates, accountID)
		}
	}
	...
	for _, candidateID := range workspaceIDCandidates {
		candidateID = strings.TrimSpace(candidateID)
		if candidateID == "" {
			continue
		}
		for _, org := range claims.CodexAuthInfo.Organizations {
			if !strings.EqualFold(strings.TrimSpace(org.ID), candidateID) {
				continue
			}
			if title := strings.TrimSpace(org.Title); title != "" {
				return title
			}
		}
	}
...
func (h *Handler) applyDefaultAuthNote(record *coreauth.Auth) {
	if record == nil || authNoteValue(record) != "" {
		return
	}
	if note := h.existingAuthNoteForRecord(record); note != "" {
		setAuthMetadataString(record, "note", note)
		return
	}
	if note := codexDefaultWorkspaceNote(record); note != "" {
		setAuthMetadataString(record, "note", note)
	}
}
