...
function invalidateWorkspaceQuotaViews() {
  window.__nicecliWorkspaceQuotaDirty = true;
  if (typeof window.invalidateWorkspaceQuotaSnapshotsCache === "function") {
    window.invalidateWorkspaceQuotaSnapshotsCache();
  }
}

async function refreshAuthDependentViews() {
  invalidateWorkspaceQuotaViews();
  await loadAuthFiles();
}

function refreshAuthDependentViewsSafely() {
  refreshAuthDependentViews().catch((error) => {
    console.error("Error refreshing auth-dependent views:", error);
  });
}
