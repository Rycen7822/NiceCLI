...
async function handleIFlowCookieSubmit(inputEl, saveBtn) {
    try {
        ...
        if (result && result.success) {
            ...
            if (typeof refreshAuthDependentViews === 'function') {
                await refreshAuthDependentViews();
            } else if (typeof loadAuthFiles === 'function') {
                await loadAuthFiles();
            }
        } else {
    ...
    try {
        await pollIFlowAuthStatus(
            ...
            () => {
                ...
                cancelIFlowAuth();
                if (typeof refreshAuthDependentViewsSafely === 'function') {
                    refreshAuthDependentViewsSafely();
                } else if (typeof loadAuthFiles === 'function') {
                    loadAuthFiles();
                }
            },
