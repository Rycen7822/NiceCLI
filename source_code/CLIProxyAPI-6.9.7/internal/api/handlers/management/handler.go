...
type Handler struct {
	...
	quotaService      *quota.CodexQuotaService
	localPassword     string
	envSecret         string
	logDir            string
	postAuthHook      coreauth.PostAuthHook
	configPersistHook func(*config.Config)
}
...
func (h *Handler) SetAuthManager(manager *coreauth.Manager) {
	...
	if h.quotaService != nil {
		h.quotaService.SetAuthManager(manager)
	}
}

// SetQuotaService attaches the quota service used by Codex quota management endpoints.
func (h *Handler) SetQuotaService(service *quota.CodexQuotaService) {
	h.quotaService = service
	if h.quotaService == nil {
		return
	}
	h.quotaService.SetConfig(h.cfg)
	h.quotaService.SetAuthManager(h.authManager)
}
...
// SetConfigPersistHook registers a callback invoked after config changes are persisted.
func (h *Handler) SetConfigPersistHook(hook func(*config.Config)) {
...
func (h *Handler) Middleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if buildinfo.IsDesktopLite() {
			localPassword := strings.TrimSpace(h.localPassword)
			if !localClient {
