...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	s.configureManagementAssets(cfg)
	auth.SetQuotaCooldownDisabled(cfg.DisableCooling)
	// Initialize management handler
	s.mgmt = managementHandlers.NewHandler(cfg, configFilePath, authManager)
	s.mgmt.SetQuotaService(quota.NewCodexQuotaService(cfg, authManager))
...
func (s *Server) setupRoutes() {
	s.registerManagementControlPanelRoute()
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/codex/quota-snapshots", s.mgmt.GetCodexQuotaSnapshots)
		mgmt.POST("/codex/quota-snapshots/refresh", s.mgmt.RefreshCodexQuotaSnapshots)
		...
		s.registerDesktopOnlyManagementRoutes(mgmt)
	}
}
...
func (s *Server) managementRoutesShouldBeEnabled(cfg *config.Config) bool {
	if strings.TrimSpace(s.localPassword) != "" {
		return true
	}
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	wasEnabled := s.managementRoutesEnabled.Load()
	shouldEnable := s.managementRoutesShouldBeEnabled(cfg)
	if shouldEnable {
		s.registerManagementRoutes()
		if !wasEnabled {
			switch {
			case strings.TrimSpace(s.localPassword) != "":
				log.Info("management routes enabled via local runtime password")
...
// SetManagementConfigPersistHook registers a callback for management config persistence.
func (s *Server) SetManagementConfigPersistHook(fn func(*config.Config)) {
	if s == nil || s.mgmt == nil {
		return
	}
	s.mgmt.SetConfigPersistHook(fn)
}
