...
func TestServerRegistersCodexQuotaManagementRoutes(t *testing.T) {
	...
	if body := rr.Body.String(); !strings.Contains(body, `"provider":"codex"`) {
		t.Fatalf("response body missing codex provider: %s", body)
	}
}

func TestDesktopLiteSkipsManagementControlPanelRoute(t *testing.T) {
	setTestFlavor(t, "desktop-lite")
...
func TestDesktopLiteSkipsManagementAPICallRoute(t *testing.T) {
	setTestFlavor(t, "desktop-lite")
...
func TestServerKeepsManagementRoutesEnabledWithLocalPasswordOnly(t *testing.T) {
	server := NewServer(
		&proxyconfig.Config{
			SDKConfig: proxyconfig.SDKConfig{
				APIKeys: []string{"test-key"},
	...
	if !server.managementRoutesEnabled.Load() {
		t.Fatal("expected management routes to be enabled from local password")
	}
	...
	if !server.managementRoutesEnabled.Load() {
		t.Fatal("expected management routes to remain enabled after config reload")
	}
}
