...
func (s *Service) ensureAuthUpdateQueue(ctx context.Context) {
	...
	if s.authUpdates == nil {
		s.authUpdates = make(chan AuthUpdate, 256)
	}
...
func (s *Service) emitAuthUpdate(ctx context.Context, update AuthUpdate) {
	if s == nil {
		return
	}
	if ctx == nil {
...
func (s *Service) handleAuthUpdate(ctx context.Context, update AuthUpdate) {
	if s == nil {
		return
	}
	s.cfgMu.RLock()
...
func (s *Service) ensureExecutorsForAuthWithMode(a *coreauth.Auth, forceReplace bool) {
	...
	case "aistudio":
		s.registerAIStudioExecutor(a.ID)
		return
	case "antigravity":
...
type desktopLiteAuthHookSkipKey struct{}

type desktopLiteCoreAuthHook struct {
	service *Service
}

func withSkipDesktopLiteAuthHook(ctx context.Context) context.Context {
	if ctx == nil {
		ctx = context.Background()
	}
	return context.WithValue(ctx, desktopLiteAuthHookSkipKey{}, true)
}

func shouldSkipDesktopLiteAuthHook(ctx context.Context) bool {
	if ctx == nil {
		return false
	}
	enabled, _ := ctx.Value(desktopLiteAuthHookSkipKey{}).(bool)
	return enabled
}

func (h *desktopLiteCoreAuthHook) OnAuthRegistered(ctx context.Context, auth *coreauth.Auth) {
	if shouldSkipDesktopLiteAuthHook(ctx) || h == nil || h.service == nil {
		return
	}
	h.service.syncBoundAuthState(auth)
}

func (h *desktopLiteCoreAuthHook) OnAuthUpdated(ctx context.Context, auth *coreauth.Auth) {
	if shouldSkipDesktopLiteAuthHook(ctx) || h == nil || h.service == nil {
		return
	}
	h.service.syncBoundAuthState(auth)
...
func (*desktopLiteCoreAuthHook) OnResult(context.Context, coreauth.Result) {}
...
func selectorForStrategy(strategy string) coreauth.Selector {
...
func (s *Service) bindDesktopLiteCoreAuthHook() {
	if s == nil || s.coreManager == nil || !buildinfo.IsDesktopLite() {
		return
	}
	s.coreManager.SetHook(&desktopLiteCoreAuthHook{service: s})
}

func (s *Service) applyConfigReload(newCfg *config.Config) {
	if s == nil {
		return
	}
	if newCfg == nil {
	...
	if s.coreManager != nil {
		s.coreManager.SetSelector(selectorForStrategy(newCfg.Routing.Strategy))
	}
...
func (s *Service) syncBoundAuthState(auth *coreauth.Auth) {
	if s == nil || s.coreManager == nil || auth == nil || auth.ID == "" {
		return
	}
	current, ok := s.coreManager.GetByID(auth.ID)
...
func isStaticManagedAuth(auth *coreauth.Auth) bool {
...
func (s *Service) syncStaticAuthSnapshot(ctx context.Context) {
	if s == nil || s.coreManager == nil {
		return
	}
	s.cfgMu.RLock()
	...
	syncCtx := withSkipDesktopLiteAuthHook(coreauth.WithSkipPersist(ctx))
	auths := SnapshotCoreAuths(cfg, authDir)
	expected := make(map[string]struct{}, len(auths))
	for _, auth := range auths {
		if auth == nil || auth.ID == "" {
			continue
		}
		expected[auth.ID] = struct{}{}
		s.applyCoreAuthAddOrUpdate(syncCtx, auth)
	}

	for _, auth := range s.coreManager.List() {
		if !isStaticManagedAuth(auth) {
			continue
		}
		if _, ok := expected[auth.ID]; ok {
			continue
		}
		s.applyCoreAuthRemoval(syncCtx, auth.ID)
	}
}
...
func (s *Service) Run(ctx context.Context) error {
	...
	s.bindDesktopLiteCoreAuthHook()
	if buildinfo.IsDesktopLite() && s.server != nil {
		s.server.SetManagementConfigPersistHook(func(newCfg *config.Config) {
			s.applyConfigReload(newCfg)
			s.syncStaticAuthSnapshot(context.Background())
		})
	}
	...
	if buildinfo.IsDesktopLite() {
		s.syncStaticAuthSnapshot(ctx)
		log.Info("desktop-lite static auth snapshot applied without fsnotify watcher")
	} else {
