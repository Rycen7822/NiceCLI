...
fn build_embedded_cliproxyapi() -> Result<(), String> {
    ...
    let easycli_root = manifest_dir
        .parent()
        .ok_or_else(|| "failed to resolve EasyCLI root".to_string())?;
    let workspace_root = easycli_root
        .parent()
        .ok_or_else(|| "failed to resolve workspace root".to_string())?;
    let cliproxy_root = workspace_root.join("CLIProxyAPI-6.9.7");
    ...
    let version = cliproxy_root
        .file_name()
        .and_then(|value| value.to_str())
        .and_then(|value| value.strip_prefix("CLIProxyAPI-"))
        .unwrap_or("embedded")
        .to_string();
