...
Write-Host "Building NiceCLI NSIS bundle"
...
$bundleDir = Join-Path $easyCliRoot "src-tauri\target\release\bundle\nsis"
$releaseExe = Join-Path $easyCliRoot "src-tauri\target\release\nicecli.exe"
...
Write-Host "NiceCLI release exe: $releaseExe"
if ($installer) {
    Write-Host "NiceCLI installer: $($installer.FullName)"
} else {
    Write-Warning "NSIS installer not found under $bundleDir"
}
